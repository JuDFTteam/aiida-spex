import sys
keyword_reference = {
    "global": ['CUSTOM', 'ALIGNBD', 'BANDOMIT', 'BLOECHL', 'BZ', 'CHKMISM', 'CHKOLAP', 'CORES', 'CORESOC', 'CUTZERO', 
    'DELTAEX', 'ENERGY', 'FIXPHASE', 'GAUSS', 'IBC', 'ITERATE', 'JOB', 'KPT', 'KPTPATH', 'MEM', 'MPIKPT', 
    'MPISPLIT', 'NBAND', 'NOSYM', 'PLUSSOC', 'RESTART', 'STOREBZ', 'TIMING', 'TRSOFF', 'WRITE', 'WRTKPT'],
    "sections": {
        'ANALYZE':['DIPOLE', 'KINETIC', 'MTACCUR', 'DOS', 'PROJECT'],
        'COULOMB':['CHKCOUL', 'LEXP', 'MULTIPOLE', 'NOSTORE', 'STEPRAD', 'TSTCOUL'],
        'LAPW':['EPAR', 'GCUT', 'LCUT', 'LO'],
        'MBASIS':['ADDBAS', 'CHKPROD', 'GCUT', 'LCUT', 'NOAPW', 'OPTIMIZE', 'SELECT', 'TOL', 'WFADJUST'],
        'SENERGY':['ALIGNVXC', 'CONTINUE', 'CONTOUR', 'DIVLOG', 'FREQINT', 'MESH', 'MPIBLK', 
        'MPISYM', 'ORDER', 'SMOOTH', 'SPECTRAL', 'VXC', 'ZERO'],
        'SUSCEP':['DISORDER', 'FPADE', 'FSPEC', 'HILBERT', 'HUBBARD', 'MULTDIFF', 'PLASMA', 'TETRAF', 'WGHTTHR'],
        'WANNIER':['BACKFOLD', 'CUTGOLD', 'DISENTGL', 'FROZEN', 'INTERPOL', 
        'IRREP', 'MAXIMIZE', 'ORBITALS', 'PLOT', 'RSITE', 'SUBSET', 'UREAD', 'WBLOCH', 'WSCALE'],
        ...
        for key2, val2 in val.items(): # key2=KS, DIELEC, etc. 
            if val2:
                ...
                if key2 in job_spectra:
                    for key3, val3 in val2.items(): # key3=R, val3={range:..., step:...}
                        spectra_range='{'
                    ...
                    for key3, val3 in val2.items(): # key3=R, val3=[...]
                        if isinstance(val3, list):
                            ...
                            job_string += f"{key3.upper()}:({','.join(map(str,band_range))}) "
        return job_string+'\n'
    else:
        return job_string+'\n'
