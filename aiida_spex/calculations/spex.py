...
class SpexCalculation(CalcJob):
    ...
    def define(cls, spec):
        ...
        spec.input('metadata.options.parser_name',
                   valid_type=six.string_types, default='spex.spexparser')
        ...
        spec.exit_code(302, 'ERROR_SPEX_CALC_FAILED',
                       message='SPEX calculation failed for unknown reason.')
        spec.exit_code(303, 'ERROR_NO_SPEXOUT',
                       message='Spex Output file was not found.')
        spec.exit_code(304, 'ERROR_SPEXOUT_PARSING_FAILED',
    ...
    def prepare_for_submission(self, folder):
        ...
        if parent_calc_folder is None:
            ...
            raise InputValidationError(
                "No parent calculation found. "
                "Need one fleurcalculation.")
        else:
        ...
        for key in settings_dict.keys():
            if key not in self._settings_keys:
                self.logger.warning("settings dict key {} for SPEX calculation"
